  ...
  for (const theme of THEMES) {
    ...
    for (const path of PAGES) {
      ...
      const r = await page.evaluate(
        ...
        { minFine: MIN_FINE_PX, minBody: MIN_BODY_PX, minTouch: MIN_TOUCH_PX },
      );
      ...
      if (mobile && r.smallTargets.length > 0) problems.push(`${where}: small touch targets (${r.smallTargets.join(', ')})`);
    }
    await ctx.close();
  }
}
